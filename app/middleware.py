...
class HTTPMethodOverrideMiddleware(object):
    ...
    This middleware will look for the header parameter first followed by the
    querystring. The default HTTP header name is `X-HTTP-METHOD-OVERRIDE` and
    the default querystring parameter name is `__METHOD__`. These can be
    changed via the constructor parameters `header_name` and
    `querystring_param` respectively. Additionally, a list of allowed HTTP
    methods may be specified via the `allowed_methods` constructor parameter.
    The default allowed methods are GET, HEAD, POST, DELETE, PUT, PATCH, and
    OPTIONS.
    """
    ...
    def _get_from_querystring(self, environ, default=None):
    ...
    def _get_method_override(self, environ):
        return environ.get(self.header_name, self._get_from_querystring(
            environ, ''))
